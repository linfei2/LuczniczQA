...
class TestQuotes:
    def test_01_get_quotes(self):
        response = requests.get("http://127.0.0.1:8080/quote/",
                                headers={
        ...
        with soft_assertions():
            ...
            assert_that(response.json()["message"]).is_equal_to("Quotes data retrieved successfully")
    ...
    def test_02_update_quote_response_200(self, create_and_delete_quote):
        ...
        update_quote = requests.put(f"http://127.0.0.1:8080/quote/{quote_id}",
                                    headers={
                                        "accept": "application/json"
                                    },
                                    json={
                                        "quote": "Tekst innego cytatu",
                                        "author": "Inny autor"
                                    })
        with soft_assertions():
        ...
        get_quote = requests.get(f"http://127.0.0.1:8080/quote/{quote_id}",
                                 headers={
                                     "accept": "application/json"
                                 })
        with soft_assertions():
            ...
            assert_that(get_quote.json()["data"][0]["quote"]).is_equal_to("Tekst innego cytatu")
            assert_that(get_quote.json()["data"][0]["author"]).is_equal_to("Inny autor")
            assert_that(get_quote.elapsed).is_less_than_or_equal_to(timedelta(milliseconds=500))
